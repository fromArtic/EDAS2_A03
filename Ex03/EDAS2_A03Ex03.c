...
void UNION(Lista *l1, Lista *l2){
    ...
    if(l1->prox + l2->prox >= MAX){
        ...
        Lista *lUnion = criarLista();
        lUnion->n = 3;

        int i, j;

        lUnion->prox = l1->prox + l2->prox;
        for(i = 0; i < l1->prox; i++){ //Copia l1 em lUnion
            lUnion->inteiros[i] = l1->inteiros[i];
        }
        for(j = 0; j < lUnion->prox; j++){ //Copia l2 em lUnion
            lUnion->inteiros[l1->prox + j] = l2->inteiros[j];
        }

        imprimirLista(lUnion);
    }
}
